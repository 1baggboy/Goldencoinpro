...
export const Login = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      await trackDeviceAndNotify(result.user.uid, result.user.email || "google-auth");
      
      // Get the Google logged in user's profile to check for 2FA
      ...
      if (err.code === "auth/popup-closed-by-user") {
        ...
        message = "This domain is not authorized for Google OAuth in Firebase console. Please authorize this URL path.";
      } else if (err.code === "auth/cancelled-popup-request") {
        message = "A login window was already opened or cancelled. Please wait or reload and try again.";
      } else {
              ...
              >
                ...
                <div className="relative my-7">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-white dark:bg-slate-900 px-3 text-slate-400 font-semibold tracking-wider">Or continue with</span>
                  </div>
                </div>
                ...
                <div className="flex w-full">
                  <button
                    ...
                    disabled={loading}
                    className="flex w-full items-center justify-center gap-3 py-3 border border-slate-200 dark:border-[#C9A96E]/10 dark:bg-[#0B0B0B] rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all font-medium text-sm text-slate-700 dark:text-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
